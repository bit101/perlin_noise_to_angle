const { Random, Num, Noise, Context, Anim } = bljs;
...
const toggle = new Toggle(panel, 20, 80, "Show Vectors", true);
...
new PlayButton(panel, 20, 120, false)
  .bind(anim, "running");
  ...
  if (toggle.toggled) {
    ...
    for (let x = 0; x < width; x += res) {
      for (let y = 0; y < height; y += res) {
        ...
        const angle = Num.map(n, -1, 1, 0, Math.PI * 2) + offset;
        context.save();
